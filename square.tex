...
from a graph $G$ drawn on a surface with selected the main
vertex $M$ and the set of auxiliary vertices $A$. We assume that no exists any edge between the auxiliary vertices. We assume also
that for each $A_i \in A$, exists at least one vertex (different from $M$) which is
connected with the auxiliary vertex $A_i$. 

By \emph{the special edges} we will understand the edges connecting $M$ and $A$;
...
\label{fig:sliding}
\end{figure}
Each corner of a vertex $A_i \in A$ can contain some special edges. 
The output of the right-handed edge sliding applied 
to the vertices $A_i \in A$ is defined as the graph $G$ 
in which these special edges from each such a corner $c$ 
of the vertex $A_i$ are shifted to the next corner $\nast(c)$
(see \cref{fig:sliding} for an example).
...
If the main vertex $M$ have not common neighbours with any vertex $A_i$,
then the right-handed edge sliding is the inverse of the left-handed edges
sliding. In addition, the edge sliding on a graph does not change the
