...
\todo{czy te twierdzenia są głównym wynikiem tej pracy? Jeśli nie, to warto to w jawny sposób powiedzieć.}
...
\todo{to nie jest dobry tekst, wymaga radykalnych środków} and its direction
(left or right). \todo[inline]{co to znaczy? czy na specjalnej krawędzi
    direction oznacza narysowanie strzałki na krawędzi? czy to znaczy, że na
    wybranym końcu krawędzi wybieramy jeden z dwóch rogów, które przylegają do
    krawędzi? a co jeśli ta krawędź jest liściem? co to znaczy w lewo albo w prawo?
    Na rysunku \cref{fig:sliding} nie ma zaznaczonej informacji o kierunku. Czy ta
    informacja jest literką L / R czy strzałką? Również na tym rysunku nie ma
    informacji o tym, który koniec jest \emph{selected}.} By \emph{the rest of the graph $G'$} we will understand the
graph $G$ with the special edges removed. \todo[inline]{ale narysowany na tej
    samej powierzchni, nawet jeśli nie jest już minimalna?} A \emph{corner} of $G'$
...
arranged in the \todo{clockwise? counterclockwise?} cyclic order (i.e., by going
...
\label{fig:sliding}
\end{figure}
\todo[inline]{prośba człowieka bez kolorowej drukarki: czy czerwone krawędzi mogłyby różnić się grubością (np.~mogą być cieńsze) od krawędzi czarnych (grubszych?)}


Each corner $c$ of the graph $G'$ can contain some selected ends 
of the special edges. The output of the edge sliding is defined 
as the graph $G$ in which these selected ends of the special edges 
in a corner $c$ are slid to the next corner $\sigma(c)$ 
if the direction is right and to the previous corner $\sigma^{-1}(c)$ 
if the direction is left (see \cref{fig:sliding} for an example).
\todo[inline]{czy wszystkie wybrane końce w danym rogu muszą mieć ten sam kierunek lewo/prawo? Ten tekst pokazuje, że użycie liczby mnogiej jest zwykle złym pomysłem i kiedy to tylko możliwe warto użyć liczby pojedynczej. } So defined edge sliding is well-defined, as long as there are no \emph{collisions}.


\todo[inline]{czy można prosić o ilustracje przedstawiające czym są kolizje? tekst poniżej aż o nie prosi.}
A \emph{collision} of a selected edge end $e$ is possible in two cases. 
\todo[inline]{Wydaje się naturalne, że kolizja dotyczy zderzenia DWÓCH rzeczy. Zderzenie dwóch samochodów. Czy może się zderzyć JEDEN samochód?}

The first case, when THERE
exists the end of a special edge on the relevant side in the same corner, which
does not slide in the same direction. The second case, when in the corner to which we
move the end $e$, THERE exists the end of a special edge which slides in the opposite
direction.

Additionally, we also assume that special edges, their ends and 
their directions are selected in such a special way that 
during sliding the edges will not collide and 
the graph obtained at the end will not contain a loop. 

The edge sliding is a bijection. 
\todo[inline]{To zaczyna być nudne: żeby to zdanie coś oznaczało, musisz powiedzieć między jakimi dwoma zbiorami jest to bijekcja.}
The inverse of the edge sliding is 
...
\todo[inline]{To zdanie sugeruje, ze edge sliding jest jakąś operacją na jakimś zbiorze. Wtedy miałoby sens mówić o operacji odwarotnej. Jaki jest sens tego zdania? Czy potrzbujesz tego zdania, czy możemy je usunąć?}
In addition, the edge sliding on a graph does not change the
number of faces of this graph.  


\todo[inline]{W długiej wersji tego tekstu zamawiam alternatywny opis ślizgania
    krawędzi oparty na bardziej precyzyjnym języku, gorąco polecam
    \url{https://arxiv.org/abs/1301.6531} jako punkt startu}
...
if using the \todo{clockwise/counterclockwise?} cyclic order of the corners on the unique face of the map
we
...
\todo[inline]{czytelnikowi będzie łatwiej, jeśli ten cały dowód a contrario zostanie zamknięty w otoczeniu lemma / proof}

We claim that $m$ has a \todo{white?} vertex of odd degree, greater than $1$. By contradiction,  suppose this is not the case. The map $m$ has at least one odd degree white vertex,
...
\todo{czy jest jakieś założenie, że $k>3$?}
...
to the vertex $w_2$, a certain number of edges connected to the vertex $w_3$. \todo{dlaczego?}
...
edges. We get a contradiction, because such maps does not exist. \todo{dlaczego?} Therefore, the
map $m$ has white vertex with an odd degree greater than $1$.

\todo{koniec dowodu}

Each corner of the vertex $b$ (inner angle in the vertex $b$ between 
two neighboring edges of the map $m$) 
\todo[inline]{czy bycie rogiem już zostało zdefiniowane wcześniej? czy to jest jakaś nowa definicja? może chcesz powiedzieć, że róg $c=(e_1,e_2)$ to para krawędzi taka, że $e_2=\nast(e_1)$?}
consists two edges $e_1$ and $e_2=\nast(e_1)$
\xout{which connect this vertex with a some vertex/vertices}. Let $T_k^{\rest}$
be the set of all the maps from the set $Y_k^{\rest}$ with a selected WHITE vertex
denoted by $w_3$ with an odd degree greater than $1$ and a 
decomposition of its
\todo{\xout{his} czy wierzchołek jest chłopakiem lub dziewczyną?} 
edges into
two non-empty, disjoint parts $P_1$ and $P_2$ such that $P_1$ has an odd
...
\end{itemize}  
\todo[inline]{pewnie prościej powiedzieć to od strony negatywnej: nie istnieje taki róg, że $e_1\in P_2$ i $e_2\in P_1$.}
THE unique example of the map from the set $T_{5}^{\rest}$ is shown in
\cref{fig:rysrestd} ($P_1$ has the blue dashed lines in the picture \todo{a gdzie jest $P_2$?}).
Clearly
\begin{align}
\label{ineqrest}
|T_{k}^{\rest}| \geq |Y_{k}^{\rest}|.
\end{align}
\todo[inline]{dlaczego? dlaczego istnieje co najmniej jeden sposób na wybór $P_1$ i $P_2$?}

\pagebreak[1] 

\section{Proof of main result \cref{???}}

\todo{tak dla pewności: co jest głównym wynikiem, jaki to numer twierdzenia?}


\todo[inline]{To miejsce prosi o jakieś spojrzenie z lotu ptaka. Czy
    skonstruujemy trzy bijekcje, a potem je połączymy przez składanie funkcji? Jaki
    jest ogólny plan i dokąd zmierzamy?}

\subsection{Three bijections}

\paragraph{THE first bijection.}
\todo{Kropka na końcu.}
We consider any map $m\in X_{k}$. As the set of special edges,
we select all edges of the vertex $b_1$ \xout{with the white vertices}.
...
its name to $w_3$, and the name of THE vertex $b_2$ to $b$. Of course, the degree of the vertex $w_3$ does not change and is odd. In
addition, $w_3$ has two distinct successors $w_1$ and $w_2$ different
than $w_3$. 
\todo{dlaczego?}
\todo[inline]{To zdanie nie jest jednoznaczne. Nie jest jasne, czy ono mówi, że $w_1$ nie jest swoim własnym następnikiem czy nie.}
We received 
\todo[inline]{warto sprawdzić subtelne różnice słownikowe między received and obtained.}
a map
from the set $T_k^{\odd}$ with a selected vertex $w_3$. Such
transformation is a bijection. \todo{dlaczego?}
...
\todo[inline]{Na rysunku \cref{fig:rysevena} zamawiam aby był bardziej w stylu
    \cref{fig:rysevenb}. Niepokoi mnie, że dwie czarne krawędzi są w wersji (a)
    zaplecione, a w wersji (b) nie. Oczywiście to jest to samo, ale niepotrzebnie
    wprowadza czytelnika w stan zakłopotania.}
...
\todo[inline]{Czy można prosić o jakiś human-readable tekst, który wyjaśnia z
    lotu ptaka co kryje się za tym ciągiem napisów?}
